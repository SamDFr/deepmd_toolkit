...
def read_vasprun_trajectory(vasprun_file: str | Path) -> list[Atoms]:
    """Read every ionic step from a VASP vasprun.xml file."""
    vasprun_file = Path(vasprun_file)
    if not vasprun_file.exists():
        raise FileNotFoundError(f"vasprun.xml file not found: {vasprun_file}")
    trajectory = read(str(vasprun_file), index=":", format="vasp-xml")
    if not trajectory:
        raise ValueError(f"No ionic steps were found in {vasprun_file}")
    return list(trajectory)


def _normalise_snapshot_index(snapshot_index: int, n_frames: int) -> int:
    """Translate Python-style negative indices into explicit frame indices."""
    normalised = snapshot_index + n_frames if snapshot_index < 0 else snapshot_index
    if normalised < 0 or normalised >= n_frames:
        raise IndexError(f"Snapshot index {snapshot_index} is out of bounds for a trajectory of length {n_frames}.")
    return normalised


def estimate_snapshot_velocities(
    trajectory: Sequence[Atoms],
    snapshot_index: int,
    ionic_timestep_fs: float,
) -> np.ndarray:
    """Estimate one frame's velocities from neighbouring positions using finite differences."""
    if ionic_timestep_fs <= 0.0:
        raise ValueError("ionic_timestep_fs must be strictly positive.")
    if len(trajectory) < 2:
        raise ValueError("At least two trajectory frames are required to estimate velocities.")

    index = _normalise_snapshot_index(snapshot_index, len(trajectory))
    current = trajectory[index]

    if index == 0:
        left = trajectory[0]
        right = trajectory[1]
        dt_fs = ionic_timestep_fs
        ...
        right = trajectory[index + 1]
        dt_fs = 2.0 * ionic_timestep_fs

    displacement = right.get_positions() - left.get_positions()
    if np.any(current.get_pbc()):
        # Apply the minimum-image convention before differentiating wrapped trajectories.
        displacement, _ = find_mic(displacement, cell=current.get_cell(), pbc=current.get_pbc())

    return displacement / dt_fs


def atoms_from_vasprun_snapshot(
    vasprun_file: str | Path,
    snapshot_index: int,
    ionic_timestep_fs: float,
) -> Atoms:
    """Load one VASP MD snapshot and attach velocities estimated by finite differences."""
    trajectory = read_vasprun_trajectory(vasprun_file)
    index = _normalise_snapshot_index(snapshot_index, len(trajectory))
    atoms = trajectory[index].copy()
    velocities = estimate_snapshot_velocities(trajectory, index, ionic_timestep_fs)
    atoms.set_velocities(velocities)
    return atoms


def extract_vasprun_snapshot_to_lammps(
    vasprun_file: str | Path,
    snapshot_index: int,
    ionic_timestep_fs: float,
    output_path: str | Path,
    ...
    """Write one `vasprun.xml` snapshot to a LAMMPS data file and return the exported Atoms object."""
    atoms = atoms_from_vasprun_snapshot(
        vasprun_file=vasprun_file,
        snapshot_index=snapshot_index,
        ionic_timestep_fs=ionic_timestep_fs,
    )
    effective_specorder = list(specorder) if specorder is not None else specorder_from_atoms(atoms)
