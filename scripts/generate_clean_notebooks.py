...
def build_inference_notebook() -> list[dict]:
    return [
        ...
        md_cell(
            ...
            Open `03_dp_correlation_plots.ipynb` and point it at the files written in `model_analysis/outputs/inference/`.
            """
        ),
    ]
    ...
    return [
        ...
        code_cell(
            ...
            if not ENERGY_FILE.exists() or not ATOM_FILE.exists():
                raise FileNotFoundError(
                    "Inference outputs were not found. Run 02_dp_inference.ipynb first or update the paths in the configuration cell."
                )
...
def main() -> None:
    ...
    # The model-analysis notebooks have diverged from this lightweight generator and are
    # maintained directly in-repo until their richer multi-run workflows are folded back here.
